...
The public WIDOCO documentation is generated from a focused ontology profile, not directly from the full LinkML OWL export. The profile in `linkml/docs/widoco-profile.yaml` defines the core classes, projected helper classes, and required property shapes. Generate and validate the WIDOCO input with:

```bash
make check-widoco-input
```

Preview the local WIDOCO site with:

```bash
make widoco-docs
make serve-widoco-docs
```

The WIDOCO HTML is post-processed so PEH term anchors use local names, e.g. `#Observation`, while canonical IRIs remain visible in the page.
