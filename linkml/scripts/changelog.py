...
@click.option('--schema', '-s', "schema_path", type=click.Path(exists=True, path_type=Path), 
              help='Path to the JSON schema file for validation')
def validate_changelog(changelog_file: Path, schema_path: Path = None) -> Tuple[bool, List[str]]:
    """
...
@click.option('--changelog-dir', '-d', required=True, 
              type=click.Path(file_okay=False, path_type=Path),
              help='Directory where changelog files will be stored')
@click.option('--version', '-v', required=False, help='Version number (e.g., 1.0.0)', default="X.X.X")
...
@click.command()
@click.option('--changelog-file', '-f', required=True, 
              type=click.Path(path_type=Path)
)
def generate_release_notes(changelog_file: Path) -> str:
    """
    ...
    for change in data.get('changes', []):
        ...
        if action == "added":
            ...
            for prop in change.get('modified_properties', []):
                prop_name = prop.get('property', '').split(':')[-1]  # Extract local name
            ...
            if replacement:
                ...
                deprecated.append(f"- **{term_id}**: Deprecated in favor of {replacement_id}. {description}")
            else:
